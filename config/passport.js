...
var LocalStrategy = require('passport-local').Strategy; //requiring the passport for local database authentication
...
passport.use(new LocalStrategy(function(username, password, done) { //This password is called from password.authenticate, from user routes.
  ...
  .exec(function(err, user) {  //executing mongo methods for mongoose
